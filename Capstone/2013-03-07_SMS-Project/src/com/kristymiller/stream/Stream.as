...
//			_nc.connect(_server);
		...
		public function selectedVideo(selVideo:String):void{
			trace("Selected Video from Landing View: " + selVideo);
			
			if(_nc){
				_nc.close();
			}
			
			_selectedVideo = selVideo;
...
//					_ns.play("guilt_trip.flv");	
		...
		public function getVideos():Array{
			...
			var folder:File = new File("/Applications/Red5/webapps/oflaDemo/streams");
			var listOfFiles:Array = folder.getDirectoryListing();
			trace("folder.directory.length: " + folder.getDirectoryListing().length);
			trace("folder.exists: " + folder.exists);
			trace("folder.getDirectoryListing(): " + folder.getDirectoryListing());
			
			for(var i:int=0; i < listOfFiles.length; i++)
			{
//				trace("listOfFiles[i] " + i + ": " + listOfFiles[i].nativePath);
				
				var str:String = listOfFiles[i].nativePath.toString();
//				trace("str: " + str);
				
				var thing:Array = str.split("/");
				videos.push(thing[thing.length-1]);
			}

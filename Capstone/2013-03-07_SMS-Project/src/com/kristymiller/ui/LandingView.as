...
{	
	...
	public class LandingView extends LandingBase
	{
		...
		private var _listOfStreams:Array;
		private var _selectStreamDD:CustomComboBox;
		...
		public function LandingView()
		{
			...
			// List of Streams Drop Down
			_selectStreamDD = new CustomComboBox();
			_selectStreamDD.width = 200;
			_selectStreamDD.prompt = "Select Video";
			
			_listOfStreams = _stream.getVideos();
			trace("List of Streams from Stream.as: " + _listOfStreams);
			
			_selectStreamDD.dataProvider = new DataProvider(_listOfStreams);
			_selectStreamDD.x = 410;
			_selectStreamDD.y = 520;
			_selectStreamDD.addEventListener(Event.CHANGE, onSelectVideoChange);
			this.addChild(_selectStreamDD);
		}
		...
		private function onSelectVideoChange(e:Event):void
		{
			trace("Select video drop down changed");
			_selectedVideo = ComboBox(e.target).selectedItem.data;
			_stream.selectedVideo(_selectedVideo);
		}
